...
+==============================================================================================*/
/*======================================================================================
| Declaration :	drv_recv
| Description : ドライブIC受信処理(ドライブIC使用Calのみ)
| Parameters  :	n/a
| Returns	  : n/a
+======================================================================================*/
void drv_recv(void)
...
void interrupt_fall_drvena(void)
{
	driveic.recv.word = 0;										// ドライブIC受信データクリア
...
void interrupt_rise_drvtck(void)
{
	if(!DENAINT_P){																// DENA => L ?
		...
		if(++cnt_extrecv == 4){													// 4bit受信(実行コマンド) ?
			if(driveic.recv.word <= 0x7){										// コマンド:0x0-0x7 ?
				switch(driveic.recv.word){
				case 0x1:														// LED1 針検
				case 0x2:														// LED2 針検
				case 0x3:														// LED3 針検
					interrupt._needle = 1;										// 針検要求
					driveic.ledno = driveic.recv.word - 1;
					break;
				}
				driveic.recv.word = 0;
				cnt_extrecv = 0;												// ドライブIC受信カウンタクリア
			...
			switch(driveic.recv.b._cmd){
			case 0x8:															// LED set1 (LED1/2 Param)
				driveic.ndet[0].byte = driveic.recv.b._param >> 5;				// LED1 Parameter
				driveic.ndet[1].byte = (driveic.recv.b._param & 0x1f) << 2;		// LED2 Parameter
				break;
			case 0x9:															// LED set2 (LED2/3 Param)
				driveic.ndet[1].byte |= (driveic.recv.b._param & 0xc00) >> 10;	// LED2 Parameter
				driveic.ndet[2].byte = (driveic.recv.b._param & 0x3ff) >> 3;	// LED3 Parameter
				normalize_sensrank_f150(driveic.ndet);							// 感度ランクの正規化
				break;
			}
			driveic.recv.word = 0;
			cnt_extrecv = 0;													// ドライブIC受信カウンタクリア
